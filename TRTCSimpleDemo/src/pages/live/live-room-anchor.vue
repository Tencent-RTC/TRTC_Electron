...
export default {
  ...
  methods: {
    ...
    showConfirmPushModal() {
      let modalId =
        this.isPushing === false ? "start-push-confirm" : "stop-push-confirm";
    ...
    async resetIconAndGetPrivilege() {
      ...
      const cameraPrivilege = await window.checkAndApplyDevicesAccessPrivilege(
        "camera"
      );
    ...
    toggleCreenShareControlBar() {
      ...
      this.screenSharingControllerBarVisiable =
        !this.screenSharingControllerBarVisiable;
    },
    ...
    async startCameraAndMic() {
      ...
      if (!cameraPrivilege || !microphonePrivilege) return false;
  ...
  async mounted() {
    ...
    // 7. 显示摄像头，开房麦克风，开始推流
    let isStart = await this.startCameraAndMic();
    // 8. 执行倒计时
    isStart && this.startLiveCountDown();
